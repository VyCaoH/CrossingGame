...
void Renderer::clear_screen(u32 color) {
	unsigned int* pixel = (u32*)render_state2.memory;
	for (int y = 0; y < render_state2.height; y++) {
		for (int x = 0; x < render_state2.width; x++) {
			*pixel++ = color;
		}
	}
}

void Renderer::draw_rect_in_pixels(int x0, int y0, int x1, int y1, u32 color) {

	x0 = clamp(0, x0, render_state2.width);
	x1 = clamp(0, x1, render_state2.width);
	y0 = clamp(0, y0, render_state2.height);
	y1 = clamp(0, y1, render_state2.height);

	for (int y = y0; y < y1; y++) {
		u32* pixel = (u32*)render_state2.memory + x0 + y * render_state2.width;
		for (int x = x0; x < x1; x++) {
			*pixel++ = color;
		}
	}
...
void Renderer::draw_arena_borders(float x, float y, float arena_x, float arena_y, u32 color) {
	x *= render_state2.height * render_scale;
	y *= render_state2.height * render_scale;
	arena_x *= render_state2.height * render_scale;
	arena_y *= render_state2.height * render_scale;

	x += render_state2.width / 2.f;
	y += render_state2.height / 2.f;
	...
	draw_rect_in_pixels(0, 0, render_state2.width, render_state2.height, color);
	draw_rect_in_pixels((int)x0, (int)y0, (int)x1, (int)y1, 0xffffffff);
	//draw_rect_in_pixels(0, render_state2.height,x1, y1, color);
	//draw_rect_in_pixels(x1, y1, render_state2.width, 0, color);
	//draw_rect_in_pixels(0, y1, x1, render_state2.height, color);
	//draw_rect_in_pixels(x0, y1, render_state2.width, render_state2.height, color);
	//draw_rect_in_pixels(x1, y0, render_state2.width, render_state2.height, color);
}

void Renderer::draw_rect(float x, float y, float half_size_x, float half_size_y, u32 color) {

	//int temp = render_state2.height * render_scale;
	x *= render_state2.height * render_scale;
	y *= render_state2.height * render_scale;
	half_size_x *= render_state2.height * render_scale;
	half_size_y *= render_state2.height * render_scale;
...
void Renderer::draw_borders(float x1, float y1, float half_size_x1, float half_size_y1, float x2, float y2, float half_size_x2, float half_size_y2, u32 color) {
	x1 *= render_state2.height * render_scale;
	y1 *= render_state2.height * render_scale;
	x1 += render_state2.width / 2.f;
	y1 += render_state2.height / 2.f;

	x2 *= render_state2.height * render_scale;
	y2 *= render_state2.height * render_scale;
	x2 += render_state2.width / 2.f;
	y2 += render_state2.height / 2.f;
...
bool Renderer::checkWall(float x)
{
	x *= render_state2.height * render_scale;
	x += render_state2.width / 2.f;
	if (x > 0)
...
void Renderer::draw_lane(float y, float half_size_x, float half_size_y, u32 color)
{
	float x0 = render_state2.width / 2 - (render_state2.width / 2 - half_size_x);
	float y0 = render_state2.height / 2 - (render_state2.height / 2 - half_size_x);

	draw_rect(0, y, x0, 0.8, 0x9C9900);
	//draw_rect(0, y-0.2, render_state2.width / 2, 0.5, 0x33AA00);
}
//x: trung tam dino, y: trung taam dino

// Nho xoa half x vaf xoa half y
void Renderer::draw_truck(float x, float y, float half_size_x, float half_size_y)
{
...
void Renderer::draw_birdL(float x, float y, float half_size_x, float half_size_y) //chieu dai
{
	//nen black
...
void Renderer::draw_birdR(float x, float y, float half_size_x, float half_size_y)
{
	//nen black
	draw_rect(x, y, 1.65, 3.25, 0x000000);
	draw_rect(x - 2.6, y + 0.1, 1.6, 1.4, 0x000000);
...
void Renderer::draw_titan(float x, float y, float half_size_x, float half_size_y)
{
	//nen black
...
void Renderer::draw_turtleR(float x, float y, float half_size_x, float half_size_y)
{
	//nen black
...
void Renderer::draw_turtleL(float x, float y, float half_size_x, float half_size_y)
{
	//nen black
	draw_rect(x, y + 2.8, 1.5, 0.3, 0x000000);
	draw_rect(x + 1.2, y + 2.5, 1.4, 0.3, 0x000000);
...
void Renderer::draw_text(const char* text, float x, float y, float size, u32 color) {
...
void Renderer::draw_entities(int number, float x, float y, float size, u32 color)
{
	float half_size = size * .5f;
	float original_y = y;
	switch (number)
...
void Renderer::draw_number(int number, float x, float y, float size, u32 color) {
	float half_size = size;// *.5f;
...
void Renderer::draw_title(float x, float y, float size, u32 color) {
	float half_size = size * .5f;
	float original_y = y;
	float original_x = x;
...
void Renderer::draw_Button(int number, float x, float y, float size, u32 color, int isHotButton) {
	float half_size = size * .5f;
	float original_y = y;
	const char** button;
	float original_x = x;
...
void Renderer::draw_Menu(float x, float y, float half_size_x, float half_size_y, int hot_button) {
	clear_screen(0x01C4FF);
	draw_title(x - 83, y + 45, 0.5, 0xE61409);
	draw_Button(BUTTON::NEW_GAME, x - 25, y + 15, 0.7, 0x000000, hot_button);
	draw_Button(BUTTON::LOAD_GAME, x - 25, y + 2, 0.7, 0x000000, hot_button);
	draw_Button(BUTTON::SETTINGS, x - 25, y - 11, 0.7, 0x000000, hot_button);
	draw_Button(BUTTON::INTRODUCTION, x - 25, y - 24, 0.7, 0x000000, hot_button);
	draw_Button(BUTTON::EXIT, x - 25, y - 37, 0.7, 0x000000, hot_button);
}
void Renderer::draw_player(float x, float y, float half_size_x, float half_size_y) {
...
void Renderer::draw_apple(int x, int y)
{
	// tao do 
...
void Renderer::draw_plant(int x, int y)
{
	//bui cay 
...
void Renderer::draw_Background(float x, float y, float max_size_x, float max_size_y)
{
	//Khung 
